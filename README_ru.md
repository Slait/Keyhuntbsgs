...
1. Ускорен Bloom на ~5-10%. Важно. Не совместим с старыми версиями Bloom.

1.0.7

1. Проанализировав обращения к FUSE-фильтру, я нашел просто критическую недоработку (узкое место) разработчика в файле keyhunt.cpp:
Внутри цикла проверки BSGS программа абсолютно всегда рассчитывала тяжелейшие криптографические хешированные функции sha256sse_33 и ripemd160sse_32.
Но фишка в том, что когда вы используете режимы фильтров FUSE или XOR (-x fuse), алгоритм на самом деле проверяет оригинальные координаты (X Point) и этот огромный кусок вычислений хеша идет просто вхолостую! Дорогое хеширование нужно только для BLOOM фильтра.
Я отредактировал keyhunt.cpp, обернув весь блок криптографии в строгое условие if (FLAGFILTERTYPE == FILTER_BLOOM).

2. При использовании режима FUSE, проверка в функции binary_fuse16_contain для отрицательной половины ключей (Negative check) всегда проверяла одни и те же нулевые адреса, бесконечно выдавая ответ false. То есть FUSE физически пропускал половину заданного вами пространства поиска BSGS!

1.0.6

1. Поправил Random

2. Поправил режим both

1.0.4
Что добавлено:

1. Поправил вывод -wif (1 | 0) для вывода WIF

2. Поправил ошибку пропусков

3. Поправил формирование Фильтра для Windows

4. Ускорил bloom через 64-bit mix

5. Ускорил формирование bloom

6. Ускорил математику AVX2

7. Векторизованная проверка Bloom, FUSE, XOR фильтра

8. Prefetching для XOR Fuse (Ускорение фильтров)

9. Huge Pages (Ускорение фильтров)

10. Ассемблер для функции double

11. Добавил blocked фильтр -blocked (скорость медленная как у Bloom, но размер компактнее, не рекомендую. Пока самый лучший это FUSE)

12. Ускорение FUSE на 25 тактов
...
2. Добавил сохранение файла пройденного прогресса 1 раз в минуту (файл save.txt).

3. Добавил XOR и Fuse Фильтры. Выбираются через -x (bloom|xor|fuse).

4. Оптимизация фильтра XOR FUSE + 10 процентов. Stage 1, 2, 3.

5. Добавил проверку checked-deep.txt. Если пройденные диапазоны из файла -D deep.txt есть в файле checked-deep.txt, то он их не проверяет.

6. Добавил очистку папки tmp перед созданием и после создания FUSE и XOR фильтра.
